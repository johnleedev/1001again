...
export default function Question (props:any) {
  ...
  const fetchDatas = async () => {
    const res = await axios.get(`${MainURL}/board/getpostsquestion/${currentPage}`);
    if (res.data) {
  ...
  return (
    <div className='board'>
      ...
      <div className="homepage_detail_titlebox">
        <p className="homepage_detail_title">문의/고충처리</p>
      </div>

      <div className="board_content">

        <div className="questionBtnbox-row">
              ...
              list.map((item:any, index:any)=>{
                ...
                return(
                  <ul className="textRow" key={index}
                    onClick={()=>{
                      navigate('/questiondetail', {state : item});  
                    }}
                  >
